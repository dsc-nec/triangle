...
void SPASpGEMM(const CSC<IT,NT> & A, const CSC<IT,NT> & B, MultiplyOperation multop, AddOperation addop, vector<IT> * RowIdsofC, vector<NT> * ValuesofC)
{
    vector<NT> spavals(A.rows);
    ...
    for(int i=0; i < B.cols; ++i)        // for all columns of B
    {
        ...
        for(IT j=B.colptr[i]; j < B.colptr[i+1]; ++j)    // For all the nonzeros of the ith column
        {
            ...
            for(IT k = A.colptr[inner]; k < A.colptr[inner+1]; ++k)
            {
                if(!spabools[A.rowids[k]])
                {
                    spabools[A.rowids[k]] = true;
                    spavals[A.rowids[k]] = A.values[k];
...
void LocalSpGEMM(const CSC<IT,NT> & A, const CSC<IT,NT> & B, MultiplyOperation multop, AddOperation addop, AlphaOperation alphaop, CSC<IT,NT> & C )
{
    ...
    if((B.cols < (PERCORECACHE / sizeof(NT) + sizeof(IT))) || omp_get_max_threads() < 2)
    {
	//cout << "Calling SPASpGEMM " << endl;
        SPASpGEMM(A, B, multop, addop, RowIdsofC, ValuesofC);
	//cout << "SPASpGEMM returned" << endl;
    }
    else
    {
