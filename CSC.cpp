...
template <class IT, class NT>
CSC<IT,NT>::CSC(graph & G, bool shuffle):nnz(G.m), rows(G.n), cols(G.n)
{
...
template <class IT, class NT>
CSC<IT,NT>::CSC(Triple<IT,NT> * triples, IT mynnz, IT m, IT n, bool shuffle):nnz(mynnz),rows(m),cols(n)
{
    colptr = new IT[cols+1]();
    rowids = new IT[nnz];
    values = new NT[nnz];
